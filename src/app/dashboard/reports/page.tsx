...
import { FileBarChart, Calendar, Sparkles, Loader2 } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';

export default function ReportsPage() {
  const { incomes, expenses, investments, loading } = useData();

  const { monthlyComparison, reports } = useMemo(() => {
    if (loading) return { monthlyComparison: [], reports: [] };
    ...
    return {
      monthlyComparison: last6Months,
      reports: activeReports,
    };
  }, [incomes, expenses, investments, loading]);
